...
{
    public class HQAuthenticationHandler : SignInAuthenticationHandler<HQAuthenticationHandlerOptions>
    {
        ...
        public HQAuthenticationHandler(IOptionsMonitor<HQAuthenticationHandlerOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock)
...

        {
            ...
            var Account = result.Principal.FindFirst("Account");

            if (Account == null)
            {
                return AuthenticateResult.Fail("No Account.");
            }

            Options.Login?.Invoke(Account.Value);
